...
st.write("Integrated multimodal automatic marketing system with Swin-Transformer, ViT-GPT2, and GPT-2.")
...
def load_pipelines():
    ...
    # 2. 图像描述 (ViT-GPT2) — 手动加载，绕过 pipeline processor 限制
    ...
    return classifier, (caption_model, caption_processor, caption_tokenizer), ad_generator
...
def generate_caption(image, caption_model, caption_processor, caption_tokenizer):
    ...
    output_ids = caption_model.generate(pixel_values, max_length=64, num_beams=4)
    caption = caption_tokenizer.decode(output_ids[0], skip_special_tokens=True)
    return caption


with st.spinner('AI 引擎启动中...'):
    v_classifier, v_captioner_bundle, t_generator = load_pipelines()
    cap_model, cap_proc, cap_tok = v_captioner_bundle
...
if uploaded_file is not None:
    ...
    with col2:
        ...
        # --- B. 运行 ViT-GPT2 生成描述 ---
        with st.spinner('ViT-GPT2 generating visual description...'):
            full_description = generate_caption(image, cap_model, cap_proc, cap_tok)
            keywords = ", ".join(full_description.split()[:10])  # 提取前10个词
    ...
    with st.expander("View Project Technical Architecture (Technical Pipeline Logic)"):
        ...
        2.  **ViT-GPT2 (Visual-Language)**: Converts image features into unstructured visual descriptions using Vision Encoder Decoder architecture.
